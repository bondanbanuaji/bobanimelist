...
import { TenraiTranslator } from '../utils/translateTenraiData';

type DataType = 'anime' | 'manga' | 'character' | 'review';

interface UseTenraiTranslationOptions {
  enabled?: boolean;
  dataType?: DataType;
}

/**
 * Custom hook untuk auto-translate Tenrai API responses
 * 
 * @example
 * const { data: animeData } = useGetAnimeByIdQuery(id);
 * const translatedData = useTenraiTranslation(animeData, { dataType: 'anime' });
 */
export function useTenraiTranslation<T>(
  data: T | undefined,
  options: UseTenraiTranslationOptions = {}
): T | undefined {
  ...
  useEffect(() => {
    ...
    const translateData = async () => {
      try {
        const translated = await TenraiTranslator.translateTenraiResponse(
          data,
          currentLang,
          dataType
        );
        ...
        if (process.env.NODE_ENV === 'development') {
          console.error('[useTenraiTranslation] Translation error:', error);
        }
...
export function useAnimeListTranslation<T extends unknown[]>(
  ...
  return useTenraiTranslation(animeList, { dataType: 'anime' });
}
...
export function useMangaListTranslation<T extends unknown[]>(
  ...
  return useTenraiTranslation(mangaList, { dataType: 'manga' });
}
...
export function useCharacterTranslation<T>(
  ...
  return useTenraiTranslation(character, { dataType: 'character' });
}
...
export function useReviewTranslation<T>(
  ...
  return useTenraiTranslation(review, { dataType: 'review' });
}
